...
class Dataset(object):
    ...
    def __init__(self, path):
        ...
        self.aesthetic_all = self.load_aesthetic_all(path + '.txt')
    ...
    def load_negative_file(self, filename):
        ...
        with open(filename, "r") as f:
            ...
            while line != None and line != "":
                arr = line.split("\t")
                negatives = []
                # arr[0]就是load_rating_file_as_list()函数读出来的正样本
    ...
    def load_aesthetic_all(self, filename):
        aesthetic_all_dict = {}
        with open(filename) as f:
            datas = f.readlines()
            for data in datas:
                aesthetic_all_dict[int(eval(data[:24]))] = np.array(list(map(eval, data[25:].split())))
        # print(len(aesthetic_all_dict))
        # print(aesthetic_all_dict[101])
        # return aesthetic_all_dict
        aesthetic_l = [i for i in range(len(aesthetic_all_dict))]
        for i in aesthetic_all_dict.items():
            aesthetic_l[i[0]] = i[1].tolist()
        return aesthetic_l
